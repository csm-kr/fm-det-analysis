...
class RCNNHead(nn.Module):
    ...
    def __init__(self, d_model, num_classes, dim_feedforward=2048, nhead=8, dropout=0.1, activation="relu", scale_clamp: float = _DEFAULT_SCALE_CLAMP, bbox_weights=(2.0, 2.0, 1.0, 1.0)):
        super().__init__()
    ...
    def forward(self, features, bboxes, pro_features, pooler, time_emb):
        ...
        obj_features = self.norm3(obj_features)                                           # [1, 4000, 256]
        
        fc_feature = obj_features.transpose(0, 1).reshape(N * nr_boxes, -1)               # [4000, 256]

        # time_feature.
        scale_shift = self.block_time_mlp(time_emb)
        scale_shift = torch.repeat_interleave(scale_shift, nr_boxes, dim=0)
        scale, shift = scale_shift.chunk(2, dim=1)
        fc_feature = fc_feature * (scale + 1) + shift                                     # [4000, 256]
