...
| **ours** (epoch 29/60, mid-train) | 2007 + 2012 | 2007 | short=800 | **70.02** |
| ours (학습 완료 후 목표) | 2007 + 2012 | 2007 | short=800 | 75~80 (예상) |

per-class AP (epoch 29 / 60, last.pt, NMS + coord fix 후 — full eval 4,952 장):
...
48.43% = chair AP
77.17% = cow AP
58.05% = diningtable AP
84.28% = dog AP
84.52% = horse AP
76.02% = motorbike AP
76.14% = person AP
47.48% = pottedplant AP
68.79% = sheep AP
57.41% = sofa AP
78.53% = train AP
75.57% = tvmonitor AP
...
> 잘하는 (>80%): cat, horse, dog · 약한 (<50%): bottle, chair, pottedplant (작거나 군집 객체). 학습 epoch 29 / 60 (48% 진행) + lr decay (milestones=47, 57) 미적용 상태. 남은 학습 + lr decay 후 추가 boost 예상.
...
## Qualitative — VOC inference overlay (epoch 29)

GT = lime, prediction (score≥0.3) = red. eval coord fix (I-10) + per-class NMS (I-11) 후 같은 가중치로 추론.
...
| ID | 카테고리 | 증상 | 해결 |
|----|---------|------|------|
| **I-11** | eval | NMS 미적용 — 한 객체에 중복 박스 → mAP 25% 천장 | `evals/{voc,coco}.py` 에 `torchvision.ops.batched_nms` per-class NMS (iou=0.5) 추가 — 같은 ckpt 0.2500 → **0.7002** |
